...
public class CardsDB 
{
	...
	static string[] typenames =new string[]{"Heal", "Damage", "Draw card(s)", "Put a card (from somewhere) in your land zone", "Put a card (from somewhere) in your hand", "Two creatures fight", "Untap", "Destroy", 
	...
	static Dictionary<int, string> typecardtext = new Dictionary<int, string>() {	{0, "Heal <t> for <p0>"}, {1, "Deal <p0> damage to <t>"}, {2, "Draw <p0> cards"}, {4, "Put <t> in your land zone"}, {5, "Put <t> in your hand"}, {6, "Two target creatures deal their attack damage to each other"}, {8, "Untap <t>"}, {9, "Destroy <t>"},   {12, "Put a <p0> in game under your control"}, {13, "Put <t> in game under your control"}, {15, "Gain <p0> mana"} };
	...
	static string[] cardtypenames =new string[]{"Land", "Creature", "Spell", "Enchantment", "Secret"};
	//
	...
	public bool DrawEffect(Effect e){
		...
		} else if (e.hastrigger) { //enchantments and lands with trigger
			GUILayout.Label("Ability triggers... ");
			e.trigger = EditorGUILayout.IntPopup(e.trigger, triggernames, triggerints, GUILayout.MaxWidth(350));
		}
		else e.trigger = -1; 
