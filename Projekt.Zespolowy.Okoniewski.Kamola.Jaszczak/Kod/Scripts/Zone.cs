...
//Skrypt obslugujacy strefy w grze
...
public class Zone : MonoBehaviour
{
    ...
    public bool FaceDown
    {
        get
        {
            if (BelongsToPlayer) return dbzone.PlayerFaceDown;
    ...
    public bool Invisible
    {
        get
        {
            if (BelongsToPlayer) return dbzone.PlayerInvisible;
    ...
    public void AddCard(card card_to_add)
    {
        ...
        if (UseSlots)
        {
            ...
            {
                if (card_to_add.ControlledByPlayer)
    ...
    void OnGUI()
    {
        ...
        {
            if (!MainMenu.DTA.settings.UseGrid) HighlightAvailableSlots();
    ...
    public void RemoveCard(card card_to_remove)
    {
        ...
        if (UseSlots)
        {
            if (card_to_remove.IsTurned && !MainMenu.DTA.settings.UseGrid)
                card_to_remove.transform.parent.Rotate(0, 0, -MainMenu.DTA.settings.OptionTurnDegrees);
    ...
    public void DrawCard()
    {
        ...
        if (BelongsToPlayer)
        {
            ...
            if (Player.HandZone == this && Player.cards_in_hand.Count == MainMenu.DTA.settings.MaxHandSize) { Player.GraveyardZone.DrawCard(); return; }
        }

        else {
            if (Enemy.Deck.Count <= 0) return;
            if (Enemy.HandZone == this && Enemy.cards_in_hand.Count == MainMenu.DTA.settings.MaxHandSize) { Enemy.GraveyardZone.DrawCard(); return; }
        }
