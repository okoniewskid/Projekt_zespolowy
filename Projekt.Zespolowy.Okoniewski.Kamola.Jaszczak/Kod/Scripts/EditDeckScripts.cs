...
public class EditDeckScripts : MonoBehaviour
{
    ...
    {
        if (card_to_move.IsZoomed) { Player.CanUnzoom = true; card_to_move.UnZoom(); }
        if (card_to_move.InCollection)
        {

            card foundcard = EditDeckScripts.cards_in_deck.Find(x => x.Index == card_to_move.Index);

            if (foundcard) { foundcard.Amount++; }
            else
            {
                GameObject clone = (GameObject)Instantiate(card_to_move.gameObject);
                EditDeckScripts.cards_in_deck.Add(clone.GetComponent<card>()); //tworzymy kopie karty celem wyswietlenia jej jako czesci talii
    ...
    //parsowanie talii gracza do Stringa
...

    {
...

        {
...

            {
                ...
                if (cards_in_deck[i].Amount <= 0)
                {
                    foreach (Transform child in cards_in_deck[i].transform) child.gameObject.GetComponent<Renderer>().enabled = false;
                }
                else //wyswietlanie karty
        ...
        {
            playerDeck.pD.Deck.Clear(); //usuwanie starej talii
        ...
        if (GUI.Button(new Rect(120, 40, 150, 50), "Table View"))
        {
            if (TableMode)
            {
                ...
                foreach (card foundcard in cards_in_collection)
                {
                    foreach (Transform child in foundcard.transform) child.gameObject.GetComponent<Renderer>().enabled = true;
        ...
        if (GUI.Button(new Rect(984, 160, 36, 50), ">"))
        {
            ...
            {
                PageCollection++; //nastepna strona kolekcji
                ArrangeCards();
            }
        }
        ...
        if (GUI.Button(new Rect(-5, 160, 36, 50), "<"))
        {
            if (PageCollection > 0)
            {
                PageCollection--; //poprzednia strona kolekcji
                ArrangeCards();
            }
        }
        ...
        if (CanExitNow) Application.LoadLevel(MainMenu.SceneNameMainMenu);//powrot do menu po zapisaniu talii
    ...
    void LoadCollection()
    {
        ...
        foreach (int foundcardID in collection)
        {
            ...
            DbCard dbcard = MainMenu.DTA.cards.Where(x => x.id == new_card.Index).SingleOrDefault();
